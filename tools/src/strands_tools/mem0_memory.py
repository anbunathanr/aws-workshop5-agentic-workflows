...
class Mem0ServiceClient:
    ...
    def store_memory(
        ...
        try:
            result = self.mem0.add(messages, user_id=user_id, agent_id=agent_id, metadata=metadata)
            print(f"DEBUG: mem0.add returned: {result}")
            return result
        except Exception as e:
            print(f"DEBUG: mem0.add failed with error: {e}")
            raise
        ...
        if action == "store":
            ...
            try:
                ...
                return ToolResult(
                    ...
                    status="success",
                    content=[ToolResultContent(text="Memory stored successfully")],
                )
                ...
                return ToolResult(
                    toolUseId=tool_use_id,
                    status="error",
                    content=[ToolResultContent(text=f"Failed to store memory: {str(e)}")],
                )
            ...
            console.print(panel)
            return ToolResult(
                toolUseId=tool_use_id, status="success", content=[ToolResultContent(text=str(memory))]
            )
            ...
            return ToolResult(
                toolUseId=tool_use_id,
                status="success",
                content=[ToolResultContent(text=str(results_list) if results_list else "No memories found")],
            )
            ...
            console.print(panel)
            return ToolResult(
                toolUseId=tool_use_id, status="success", content=[ToolResultContent(text=str(history))]
            )
